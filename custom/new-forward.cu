...
__global__ void conv_forward_kernel(float *y, const float *x, const float *k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    */
    ...
    int m, h, w, c, p, q;
    m = blockIdx.x;
    h = (blockIdx.y / W_grid) * blockDim.y + threadIdx.y;
    w = (blockIdx.y % W_grid) * blockDim.x + threadIdx.x;
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_y, const float *host_x, const float *host_k, float **device_y_ptr, float **device_x_ptr, float **device_k_ptr, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    cudaMemcpy(*device_k_ptr, host_k, C * M * K * K * sizeof(float), cudaMemcpyHostToDevice);
...
__host__ void GPUInterface::conv_forward_gpu(float *device_y, const float *device_x, const float *device_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    // H/TILE_WIDTH number of horizontal tiles per output map 
    // H/TILE_WIDTH number of vertical tiles per output map

    dim3 blockDim(TILE_WIDTH, TILE_WIDTH, 1);
    // dim3 gridDim(B, ceil(M/(1.0 * TILE_WIDTH)), ceil(H_out*W_out/(1.0 * TILE_WIDTH)));
    dim3 gridDim(M, Z, 1);
    // printf("%f ",gridDim(B, M, Z));
